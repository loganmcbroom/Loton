...
PVOCDesampleModel::PVOCDesampleModel()
	: PVOCProcessModel()
	, factorModel( new NumberSliderModel( 30, 1, NumberSlider::infinity ) )
...
bool PVOCDesampleModel::process()
	{
	...
	setFunctor( [in, factor, interp, c = canceller]()
		{
		return std::shared_ptr<NodeData>( new PVOCData( in.desample( factor->f, interp->f, *c ) ) );
		});

	return true;
	}

QString PVOCDesampleModel::caption() const { return "Desample"; }
QString PVOCDesampleModel::name() const { return "PVOC Desample"; }
QString PVOCDesampleModel::portCaption( PortType type, PortIndex index ) const
	{
	if( type == PortType::In )
		switch( index )
			{
...
unsigned int PVOCDesampleModel::nPorts( PortType type ) const
	{
	switch( type )
		{
...
NodeDataType PVOCDesampleModel::dataType( PortType type, PortIndex index ) const
	{
	if( type == PortType::In )
		switch( index )
			{
...
ControllerPairs PVOCDesampleModel::makeInputControllers()
	{
	return {
...
QJsonObject PVOCDesampleModel::save() const
	{
...
void PVOCDesampleModel::restore( QJsonObject const & p )
	{
