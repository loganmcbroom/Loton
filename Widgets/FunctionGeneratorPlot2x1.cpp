#include "FunctionGeneratorPlot2x1.hpp"
...
FunctionGeneratorPlot2x1Model::FunctionGeneratorPlot2x1Model( const QString & _title, QObject * parent )
	: QObject( parent )
	, out( nullptr )
	, title( _title )
	{
...
FunctionGeneratorPlot2x1Model::~FunctionGeneratorPlot2x1Model()
	{
	}

bool FunctionGeneratorPlot2x1Model::getLogMode() const
	{
	return logMode;
	}

void FunctionGeneratorPlot2x1Model::setLogMode( bool newMode )
	{
	logMode = newMode;
...
void FunctionGeneratorPlot2x1Model::setOut( std::shared_ptr<Func2x1Data> newOut )
	{
	out = newOut;
	emit dataUpdated();
	}
...
FunctionGeneratorPlot2x1View::FunctionGeneratorPlot2x1View( FunctionGeneratorPlot2x1Model * _model, int evalFreq, QWidget * parent )
	: QWidget( parent )
	, model( _model )
	, plot( new ControllableLogPlot2x1( model->getLogMode() ) )
	...
	QObject::connect( model, &FunctionGeneratorPlot2x1Model::logModeUpdated, plot, &ControllableLogPlot2x1::setLogMode );
	...
	QObject::connect( plot, &ControllableLogPlot::viewChanged,
					this, &FunctionGeneratorPlot2x1View::regraph );

	QObject::connect( model, &FunctionGeneratorPlot2x1Model::dataUpdated,
					this, &FunctionGeneratorPlot2x1View::regraph );
...
FunctionGeneratorPlot2x1View::~FunctionGeneratorPlot2x1View() {}

/*
 * Uses the contents of out to update and redraw the curve and graph
 */
void FunctionGeneratorPlot2x1View::regraph()
	{
	...
	fs = std::make_unique<QFuture<FutureType>>( QtConcurrent::run(
		...
		for( float y = modYInterval.start; y <= modYInterval.end; y = modYInterval( y ) )
			for( float x = modXInterval.start; x <= modXInterval.end; x = modXInterval( x ) )
				newPoints.push_back( { x, y, fCopy( x, y ) } );
...
FunctionGeneratorPlot2x1ViewAdv::FunctionGeneratorPlot2x1ViewAdv( FunctionGeneratorPlot2x1Model * _model, int ef, QWidget * parent )
	: FunctionGeneratorPlot2x1View( _model, ef, parent )
	{
